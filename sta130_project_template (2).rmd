...
neighbourhood %>%
  ...
  ggtitle("Number of Autothefts based on Spring/Summer \nand Fall/Winter")+
...
**Results:** No evidence against our null hypothesis
...
neighbourhood %>% 
  ...
  ggtitle("Number of Autothefts based on Day time \nvs Night time")+
  labs(x= "Day time vs Night time", y= "Number of Autothefts")+
  theme(axis.text.x = element_text(size = 14), axis.title.x = element_text(size = 16),
        axis.text.y = element_text(size = 14), axis.title.y = element_text(size = 16),
        plot.title = element_text(size = 18, face = "bold", hjust = 0.5))
```
...
sim %>% ggplot(aes(x = p_day)) +
  geom_histogram(binwidth = 0.001, colour = "black", fill = "lightblue") + ggtitle("Simulated Distribution of daytime") + labs(x = "Proportion day time in 18175 auto thefts" )
```
...
**Null Hypothesis:** The proportion of auto thefts occuring at midnight between spring/summer seasons and fall/winter seaons are the same.

**Results:** The classification tree predicts the likelihood of crimes occuring at what time and what season; specifically late night crimes are predicted to be fall/winter crimes and daytime crimes are predicted to be spring/summer.
